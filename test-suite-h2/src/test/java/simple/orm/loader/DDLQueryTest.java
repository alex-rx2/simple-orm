...
public class DDLQueryTest extends BaseH2Test {
    ...
    public void testCreateTable() throws SQLException {
        // test
        {
            ...
            conn.executeDDLQuery(QLOADER.loadQuery(
                    QueryType.DDL,
                    QuerySource.of("CREATE TABLE test_table (id INT PRIMARY KEY, some_value VARCHAR(255) NULL)"),
    ...
    public void testExecuteAnyQuery() throws SQLException {
        ...
        {
            Connection conn = database.connect(10);
            conn.executeAnyQuery(QLOADER.loadQuery(
                    QueryType.DDL,
                    QuerySource.of("CREATE TABLE test_table (id INT PRIMARY KEY, some_value VARCHAR(255) NULL)"),
                    InjectionStrategy.none(),
                    ExtractionStrategy.noneDdl()
            ...
            conn.executeAnyQuery(QLOADER.loadQuery(
                    QueryType.DDL,
                    QuerySource.of("ALTER TABLE test_table ADD COLUMN some_number INT NOT NULL"),
                    InjectionStrategy.none(),
                    ExtractionStrategy.noneDdl()
            ));
            conn.close();
