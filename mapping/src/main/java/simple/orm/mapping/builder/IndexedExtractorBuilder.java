...
public class IndexedExtractorBuilder {
    ...
    public IndexedExtractorBuilder param(String label, TypeMapper<?, ?> mapper) {
        if (mapper == null) {
            throw new NullPointerException("mapper is null");
        }
        params.apply(p -> p.append(IndexedParameter.of(label, mapper)));
        return this;
    }

    public IndexedExtractorBuilder param(String label, ParamInfo<?, ?> param) {
        if (param == null) {
            throw new NullPointerException("param is null");
        }
        params.apply(p -> p.append(IndexedParameter.of(label, param)));
        return this;
    }

    public IndexedExtractorBuilder param(String label,
                                         String mapperName,
                                         ParameterJdbcType<?> jdbcType,
                                         Class<?> javaType,
                                         String tag
